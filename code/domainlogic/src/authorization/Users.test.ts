...
describe("User Tests using Firebase " + config.testEnvironment, function() {
  ...
  const testFunc4 = () => (async () => {
    ...
    config.authenticationState = TestAuthData.owner;
    callState = await CSU.getCallStateForCurrentUser(config.dataStorageState, config.authenticationState);
    const owner = await OpaDb.Users.queries.getByIdWithAssert(config.dataStorageState, TestAuthData.owner.opaUserId);
    const ownerRole = await OpaDb.Roles.queries.getByIdWithAssert(config.dataStorageState, owner.assignedRoleId);
    const ownerDM = await Users.convertUserToDisplayModel(callState, owner);
    const ownerDMR = await Users.convertUserToDisplayModel(callState, owner, ownerRole);
    expect(ownerDM.id).equals(owner.id);
    expect(ownerDM.assignedRole.id).equals(ownerRole.id);
    expect(ownerDMR.id).equals(owner.id);
    expect(ownerDMR.assignedRole.id).equals(ownerRole.id);
    const admin = await OpaDb.Users.queries.getByIdWithAssert(config.dataStorageState, TestAuthData.admin.opaUserId);
    const adminRole = await OpaDb.Roles.queries.getByIdWithAssert(config.dataStorageState, admin.assignedRoleId);
    const adminDM = await Users.convertUserToDisplayModel(callState, admin);
    const adminDMR = await Users.convertUserToDisplayModel(callState, admin, adminRole);
    expect(adminDM.id).equals(admin.id);
    expect(adminDM.assignedRole.id).equals(adminRole.id);
    expect(adminDMR.id).equals(admin.id);
    expect(adminDMR.assignedRole.id).equals(adminRole.id);
    const editor = await OpaDb.Users.queries.getByIdWithAssert(config.dataStorageState, TestAuthData.editor.opaUserId);
    const editorRole = await OpaDb.Roles.queries.getByIdWithAssert(config.dataStorageState, editor.assignedRoleId);
    const editorDM = await Users.convertUserToDisplayModel(callState, editor);
    const editorDMR = await Users.convertUserToDisplayModel(callState, editor, editorRole);
    expect(editorDM.id).equals(editor.id);
    expect(editorDM.assignedRole.id).equals(editorRole.id);
    expect(editorDMR.id).equals(editor.id);
    expect(editorDMR.assignedRole.id).equals(editorRole.id);
    const viewer = await OpaDb.Users.queries.getByIdWithAssert(config.dataStorageState, TestAuthData.viewer.opaUserId);
    const viewerRole = await OpaDb.Roles.queries.getByIdWithAssert(config.dataStorageState, viewer.assignedRoleId);
    const viewerDM = await Users.convertUserToDisplayModel(callState, viewer);
    const viewerDMR = await Users.convertUserToDisplayModel(callState, viewer, viewerRole);
    expect(viewerDM.id).equals(viewer.id);
    expect(viewerDM.assignedRole.id).equals(viewerRole.id);
    expect(viewerDMR.id).equals(viewer.id);
    expect(viewerDMR.assignedRole.id).equals(viewerRole.id);
    const guest = await OpaDb.Users.queries.getByIdWithAssert(config.dataStorageState, TestAuthData.guest.opaUserId);
    const guestRole = await OpaDb.Roles.queries.getByIdWithAssert(config.dataStorageState, guest.assignedRoleId);
    const guestDM = await Users.convertUserToDisplayModel(callState, guest);
    const guestDMR = await Users.convertUserToDisplayModel(callState, guest, guestRole);
    expect(guestDM.id).equals(guest.id);
    expect(guestDM.assignedRole.id).equals(guestRole.id);
    expect(guestDMR.id).equals(guest.id);
    expect(guestDMR.assignedRole.id).equals(guestRole.id);
    const testUser = await OpaDb.Users.queries.getByIdWithAssert(config.dataStorageState, TestAuthData.testUser.opaUserId);
    const testUserRole = await OpaDb.Roles.queries.getByIdWithAssert(config.dataStorageState, testUser.assignedRoleId);
    const testUserDM = await Users.convertUserToDisplayModel(callState, testUser);
    const testUserDMR = await Users.convertUserToDisplayModel(callState, testUser, testUserRole);
    expect(testUserDM.id).equals(testUser.id);
    expect(testUserDM.assignedRole.id).equals(testUserRole.id);
    expect(testUserDMR.id).equals(testUser.id);
    expect(testUserDMR.assignedRole.id).equals(testUserRole.id);
    ...
    config.authenticationState = TestAuthData.owner;
    callState = await CSU.getCallStateForCurrentUser(config.dataStorageState, config.authenticationState);
    const userDMsOwnerAll = await Users.convertUsersToDisplayModels(callState, usersOwnerAll);
    expect(userDMsOwnerAll.length).equals(usersOwnerAll.length);
    userDMsOwnerAll.forEach((value, index) => expect(value.id).equals(usersOwnerAll[index].id));
    const userDMsAdminAll = await Users.convertUsersToDisplayModels(callState, usersAdminAll);
    expect(userDMsAdminAll.length).equals(usersAdminAll.length);
    userDMsAdminAll.forEach((value, index) => expect(value.id).equals(usersAdminAll[index].id));
    const userDMsEditorAll = await Users.convertUsersToDisplayModels(callState, usersEditorAll);
    expect(userDMsEditorAll.length).equals(usersEditorAll.length);
    userDMsEditorAll.forEach((value, index) => expect(value.id).equals(usersEditorAll[index].id));
    const userDMsViewerAll = await Users.convertUsersToDisplayModels(callState, usersViewerAll);
    expect(userDMsViewerAll.length).equals(usersViewerAll.length);
    userDMsViewerAll.forEach((value, index) => expect(value.id).equals(usersViewerAll[index].id));
    const userDMsGuestAll = await Users.convertUsersToDisplayModels(callState, usersGuestAll);
    expect(userDMsGuestAll.length).equals(usersGuestAll.length);
    userDMsGuestAll.forEach((value, index) => expect(value.id).equals(usersGuestAll[index].id));
    const userDMsTestUserAll = await Users.convertUsersToDisplayModels(callState, usersTestUserAll);
    expect(userDMsTestUserAll.length).equals(usersTestUserAll.length);
    userDMsTestUserAll.forEach((value, index) => expect(value.id).equals(usersTestUserAll[index].id));
  });
